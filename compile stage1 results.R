...
for (c in 1:N.countries){
  ...
  if(subnational[c]==0){  #National-level only
  ...
  } else {    ## if country contains state level
    ...
    #Exclusions for each country, if any
    if (grepl("Brazil", country, fixed=TRUE)) {
      exclude.obs<-grep("AA", dimnames(log_rr_q)[[3]])  
    }else if (grepl("Chile", country, fixed=TRUE)) {
      exclude.obs<-c(3,6)  
    }else if (grepl("Mexico", country, fixed=TRUE)) {
      exclude.obs<-grep("XX", dimnames(log_rr_q)[[3]])  
    }else if (grepl("Ecuador", country, fixed=TRUE)) {
      exclude.obs<-grep("_50", dimnames(log_rr_q)[[3]])  
    }else{
      exclude.obs=0
    }
    select.obs.state<-setdiff(select.obs,c(exclude.obs,exclude.obs2))      
