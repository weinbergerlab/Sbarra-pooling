...
```{r viewdata, include=TRUE}
    ds<-read.csv('./prelog_Chile_state_u2.csv')
  head(ds)
...
analysis <- evaluatr.init(
  country = "Chile_state", data = ds,
  post_period_start = "2011-01-01", #First 'post-intervention' month is Jan 2012
  eval_period_start = "2012-01-01", #We ignore first 12 month of data to allow for vaccine ramp up
