...
country       <- 'Denmark' #Country or region name.
...
input_directory  <- 'C:/Users/dmw63/Dropbox (Personal)/Meta analysis datasets/Denmark/' #Directory (or URL) containing input data file.
file_name="prelog_Denmark_processed_data.csv"
...
start_date        <- as.Date('1998-06-01') #Indicates the date of the first data point.
intervention_date <- as.Date('2007-10-01') #Indicates the date of intervention in the data.
end_date          <- as.Date('2013-12-01') #Indicates the date of the last data point.
pre_period        <- as.Date(c('1998-06-01', '2007-09-30')) #Range over which the data is trained for the CausalImpact model.
post_period       <- as.Date(c('2007-10-01', '2013-12-01')) #Range from the intervention date to the end date.
eval_period       <- as.Date(c('2009-10-01', '2013-12-01')) #Range over which rate ratio calculation will be performed.
