...
output_directory<- paste0(dirname(getwd()), "/Results CP sim no shrinkage/")
...
source('./simulation/model single country no shrinkage.R')  #Read in model
...
posterior_samples<-coda.samples(model_jags, 
                                variable.names=c("reg_mean",'reg_unbias' ,'cp1','cp2',"beta", 'gamma','lambda'),
                                thin=10,
                                n.iter=15000)
...
##################################################################
 dic1<-dic.samples(model_jags, 5000) 
 #dic2<-dic.samples(model_jags, 5000)
saveRDS(dic1,paste0(output_directory,'dic_CP.rds'))
